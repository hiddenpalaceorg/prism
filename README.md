...
Prism analyzes disc images and ROMs for game preservation. 
...
<p>
  <img src="docs/screenshot-macos.png" height="300" alt="Prism on macOS">
  <img src="docs/screenshot-windows.png" height="300" alt="Prism on Windows">
</p>
...
```
crates/prism-core/     Rust engine: schema, adapter driver, fingerprints, cache, SQLite, DAT/JSON
crates/prism-cli/      command-line interface
crates/prism-ffi/      UniFFI bridge over the core, used by the native GUIs
...
```sh
cd ps2exe-adapter && uv run --group dev pyinstaller prism-adapter.spec
prism --adapter-bin dist/prism-adapter analyze image.bin
```
...
the CLI; the only system dependency is libarchive. `scripts/prism-build-set.sh`
analyzes whole dump-set directories in parallel and exports an ingestable feed:

```sh
scripts/prism-build-set.sh analyze --bin target/release/prism \
  --adapter ps2exe-adapter --lib ~/prism-lib/main --jobs 6 -- /path/to/dumps
scripts/prism-build-set.sh export --bin target/release/prism \
  --lib ~/prism-lib/main --out feed.jsonl
```

export function ContainerInfo({ info }: { info: any }) {
  // console.log(info);
  ...
  return (
    <div>
      <table>
        <tbody>
          <tr>
            <td className="pr-4">System</td>
            <td>
              <div className="flex gap-1">
                {info.system.name === 'pc' && <PCIcon className="size-4" />}
                {info.system.name?.toUpperCase()}
              </div>
            </td>
          </tr>
          <tr>
            <td className="pr-4">Executable</td>
            <td>{info.exe.filename} ({info.exe.date})</td>
          </tr>
        </tbody>
      </table>
    </div>
...
function PCIcon({ className }: { className?: string }) {
  return (
    <svg

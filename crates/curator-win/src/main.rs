...
//!   cargo check --manifest-path crates/curator-win/Cargo.toml --target x86_64-pc-windows-gnu

#![cfg_attr(windows, windows_subsystem = "windows")]

#[cfg(not(windows))]
fn main() {
    eprintln!("curator-win targets Windows; build with --target x86_64-pc-windows-*");
}

#[cfg(windows)]
fn main() -> windows::core::Result<()> {
    // `curator-win --cli <command…>` runs the shared curator CLI instead of
    ...
    if args.get(1).map(String::as_str) == Some("--cli") {
        args.remove(1);
        app::attach_console();
        std::process::exit(curator_cli::run(args, app::cli_fallback_adapter()));
    }
    app::run()
}

//! UniFFI bridge: a thin, UI-agnostic facade over [`prism_core`] for native
...
//! cargo run -p prism-ffi --bin uniffi-bindgen -- generate \
//!     --library target/debug/libprism_ffi.dylib --language swift --out-dir macos/Generated
//! ```
...
use prism_core::{render, Analysis, Analyzer, Config, Event, Node, ProgressObserver};
...
impl From<prism_core::Error> for PrismError {
    fn from(e: prism_core::Error) -> Self {
        match e {
            prism_core::Error::Cancelled => PrismError::Cancelled,
            other => PrismError::Failed { message: other.to_string() },
        }
    }
}
...
impl From<LibrarySort> for prism_core::db::LibrarySort {
    fn from(s: LibrarySort) -> Self {
        match s {
            LibrarySort::Name => prism_core::db::LibrarySort::Name,
            LibrarySort::System => prism_core::db::LibrarySort::System,
            LibrarySort::Files => prism_core::db::LibrarySort::Files,
            LibrarySort::Size => prism_core::db::LibrarySort::Size,
            LibrarySort::Date => prism_core::db::LibrarySort::Date,
        }
    }
}
...
) -> Result<AnalysisSummary, PrismError> {
    let record = &analysis.record;
    let xml = render::to_dat_xml(record);
    let json = render::to_json(record).map_err(PrismError::from)?;
...
fn entry_from_row(r: prism_core::db::LibraryRow) -> LibraryEntry {
    LibraryEntry {
        sha256: r.sha256,
        name: r.name,
        system: r.system,
...
pub fn folder_is_single_build(root: String) -> bool {
    prism_core::folder_is_single_build(std::path::Path::new(&root))
}
...
pub struct Engine {
    ...
    reader: Mutex<prism_core::Reader>,
}
...
impl Engine {
    ...
    pub fn recent_builds(&self, limit: u32) -> Result<Vec<LibraryEntry>, PrismError> {
    ...
    pub fn search_library(
        ...
        limit: u32,
        offset: u32,
    ) -> Result<Vec<LibraryEntry>, PrismError> {
    ...
    pub fn library_systems(&self) -> Result<Vec<String>, PrismError> {
    ...
    pub fn list_files(&self, root: String) -> Result<Vec<String>, PrismError> {
        Ok(prism_core::list_import_units(std::path::Path::new(&root))
            .into_iter()
    ...
    pub fn load_build(&self, sha256: String) -> Result<Option<AnalysisSummary>, PrismError> {
    ...
    pub fn export_jsonl(&self, out_path: String) -> Result<u64, PrismError> {
        let file = std::fs::File::create(&out_path)
            .map_err(|e| PrismError::Failed { message: format!("creating {out_path}: {e}") })?;
    ...
    pub fn export_bundle(&self, out_path: String) -> Result<u64, PrismError> {
        Ok(self
            .inner
            .lock()

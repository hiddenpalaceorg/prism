...
final class AppModel: ObservableObject {
    ...
    private let service = PrismService()
    ...
    private let analysisQueue = DispatchQueue(label: "prism.analysis")
    ...
    private func resolveAdapter() -> (dir: String?, bin: String?) {
        ...
        if let bin = env["PRISM_ADAPTER_BIN"] { return (nil, bin) }
        // Shipped app: Prism.app/Contents/Resources/adapter/prism-adapter
        if let res = Bundle.main.resourceURL {
            let launcher = res.appendingPathComponent("adapter/prism-adapter")
            if FileManager.default.isExecutableFile(atPath: launcher.path) {
                return (nil, launcher.path)
            }
        }
        return (env["PRISM_ADAPTER_DIR"] ?? "../ps2exe-adapter", nil)
    }
    ...
    private func makeEngine() throws -> Engine {
        ...
        let e = try Engine(
            adapterDir: dir,
            adapterBin: bin,
            dataDir: ProcessInfo.processInfo.environment["PRISM_DATA_DIR"]
        )
        engine = e
    ...
    private func materializeAsset(_ asset: AssetInfo, blob: String) throws -> URL {
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("prism-assets", isDirectory: true)
    ...
    func exportLibrary() {
        ...
        panel.prompt = "Export"
        panel.nameFieldStringValue = "collection.prism.zip"
